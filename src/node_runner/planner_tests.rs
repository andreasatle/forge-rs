...
fn processor_with_provides(provides: &[String]) -> PlannerOutputProcessor<'_> {
    PlannerOutputProcessor::new(&[], provides)
}

/// Builds a `task_kv` map from `(key, value)` pairs, for tests exercising
/// [`validate_task_kv`] against a specific `provides` schema.
...
#[test]
fn task_kind_task_missing_declared_provides_key_fails_validation() {
    // Invariant: `kind: "task"` tasks must carry every key the adapter
    // declares in `provides` — a key present in `provides` but absent from
    // this task's `task_kv` is `MissingTaskKey`, a retryable validation
    // failure rather than a silently accepted gap.
    let provides = vec![
        "name".to_string(),
    ...
    let output = PlannerOutput {
        ...
        tasks: vec![PlannerTask {
            ...
            targets: vec![],
            depends_on: vec![],
            task_kv: kv(&[("function_name", "fibonacci"), ("file_path", &file_path())]),
        }],
    };
    assert_eq!(
        processor_with_provides(&provides).validate(&output),
        Err(PlannerValidationError::MissingTaskKey {
...
#[test]
fn task_kind_task_with_blank_declared_value_fails_validation() {
    // Invariant: a declared key present but blank (or whitespace-only) is
    // treated the same as absent — `MissingTaskKey`, not silently accepted.
    let provides = vec!["name".to_string()];
    let output = PlannerOutput {
        kind: PlannerOutputKind::Task,
        tasks: vec![PlannerTask {
            id: "sub-a".to_string(),
...
#[test]
fn task_kind_task_with_all_declared_keys_passes_validation() {
    // Invariant: a task carrying exactly the adapter's declared `provides`
    // keys, each non-blank, passes validation.
    let provides = vec![
        "name".to_string(),
        "function_name".to_string(),
        "file_path".to_string(),
    ...
    assert!(
        processor_with_provides(&provides).validate(&output).is_ok(),
        "task carrying exactly the declared provides keys must pass validation"
    );
}

#[test]
fn task_kind_task_with_unrecognized_key_fails_validation() {
    // Invariant: a `task_kv` key not present in the adapter's declared
    // `provides` at all is rejected outright (`UnknownTaskKey`) — distinct
    // from a declared-but-missing key (`MissingTaskKey` above).
    let provides = vec!["file_path".to_string()];
    let output = PlannerOutput {
        kind: PlannerOutputKind::Task,
        tasks: vec![PlannerTask {
            id: "sub-a".to_string(),
            ...
            targets: vec![],
            depends_on: vec![],
            task_kv: kv(&[("file_path", &file_path()), ("bogus_key", "x")]),
        }],
    };
    assert_eq!(
        processor_with_provides(&provides).validate(&output),
        Err(PlannerValidationError::UnknownTaskKey {
            task_id: "sub-a".to_string(),
...
#[test]
fn task_kind_task_with_empty_provides_imposes_no_task_kv_requirement() {
    // Invariant: an adapter that declares no `provides` at all imposes no
    // `task_kv` requirement — a task with an empty `task_kv` still passes.
    let output = PlannerOutput {
        kind: PlannerOutputKind::Task,
        tasks: vec![PlannerTask {
            id: "sub-a".to_string(),
    ...
    assert!(
        validate_planner_output(&output).is_ok(),
        "empty provides must impose no task_kv requirement"
    );
}
...
#[test]
fn task_kind_output_carries_task_kv_into_plan_tasks() {
    // Invariant: `PlannerTask::task_kv` is carried verbatim into
    // `PlannerTaskOutput::task_kv`, alongside `id`/`objective`, so the
    // manifest recording path (`IntegrationService::integrate_plan_tasks`)
    // has access to it.
    let output = PlannerOutput {
        kind: PlannerOutputKind::Task,
        tasks: vec![PlannerTask {
            ...
            targets: vec![],
            depends_on: vec![],
            task_kv: kv(&[("name", "sub_a")]),
        }],
    };
    let plan = planner_output_to_plan_output(output);
    assert_eq!(
        plan.tasks[0].task_kv.get("name").map(String::as_str),
        Some("sub_a")
    );
}
...
#[test]
fn plan_kind_task_missing_declared_provides_key_fails_validation() {
    // Invariant: `MissingTaskKey` validation covers `kind: "plan"` tasks, not
    // just `kind: "task"` — a `kind: "plan"` batch can collapse into a
    // terminal task row via the single-task short-circuit (see
    // `plan_kind_output_with_single_task_becomes_terminal_task`), so a
    // missing declared key must be caught here, at validation time, rather
    // than surfacing later as a downstream `.expect()` panic
    // (`triggers::task_target_files`).
    let provides = vec!["name".to_string()];
    let output = PlannerOutput {
        kind: PlannerOutputKind::Plan,
        tasks: vec![PlannerTask {
...
#[test]
fn short_circuited_single_task_plan_output_carries_task_kv_through() {
    ...
    // `into_plan` carries its planner-decided `task_kv` all the way through
    // — exactly the values a `ForTasks`-spawned node reads directly in
    // `triggers::task_target_files` once the row lands in the manifest, with
    // no name-based re-derivation.
    let provides = vec![
        "name".to_string(),
        "function_name".to_string(),
        "file_path".to_string(),
    ...
    assert!(
        processor_with_provides(&provides).validate(&output).is_ok(),
        "a single-task plan output with all declared keys must pass validation"
    );
    let plan = planner_output_to_plan_output(output);
    assert!(plan.children.is_empty());
    assert_eq!(plan.tasks.len(), 1);
    ...
    assert_eq!(
        plan.tasks[0].task_kv.get("file_path").map(String::as_str),
