...
pub struct TaskRecord {
    ...
    /// `file_path`), carried from `PlannerTaskOutput::task_kv`. Empty for
    /// rows recorded via `record_task` (completed `Work` nodes), which have
    /// no corresponding `PlannerTask` and thus no metadata to carry.
